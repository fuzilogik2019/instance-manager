...
import { Shield, Plus, Trash2, Server, Gamepad2, Globe, Save } from 'lucide-react';
import { SecurityGroup, SecurityGroupRule } from '../../types/aws';
import { updateSecurityGroup } from '../../services/api';
...
interface SecurityGroupEditFormProps {
  securityGroup: SecurityGroup;
  onSecurityGroupUpdated: () => void;
  onClose: () => void;
}

export default function SecurityGroupEditForm({ securityGroup, onSecurityGroupUpdated, onClose }: SecurityGroupEditFormProps) {
  const [updating, setUpdating] = useState(false);
  const [rules, setRules] = useState<Omit<SecurityGroupRule, 'id'>[]>(
    securityGroup.rules.map(rule => ({
  ...
  const onSubmit = async (data: FormData) => {
    try {
      setUpdating(true);
      await updateSecurityGroup(securityGroup.id, {
        name: data.name,
        description: data.description,
        rules,
      });
      onSecurityGroupUpdated();
      onClose();
    } catch (error) {
  ...
  const isDefault = securityGroup.name === 'default';

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Edit Security Group</h2>
          <p className="text-sm text-gray-600 mt-1">Modify firewall rules for {securityGroup.name}</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input
              ...
              disabled={isDefault}
              helpText={isDefault ? 'Default security group name cannot be changed' : undefined}
            />
            ...
            <Input
              ...
              disabled={isDefault}
              helpText={isDefault ? 'Default security group description cannot be changed' : undefined}
            />
          </div>

          <Card title="Quick Add - Common Ports">
            ...
            <div className="space-y-4">
              ...
              {rules.length === 0 && (
                <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
                  ...
                  <p className="text-sm text-gray-500 mt-1">This will block all incoming traffic</p>
                </div>
              )}
            </div>
          </Card>
          ...
          <div className="bg-yellow-50 p-4 rounded-lg">
            <h4 className="font-medium text-yellow-900 mb-2">⚠️ Important Security Notice</h4>
            <ul className="text-sm text-yellow-800 space-y-1">
              <li>• Changes to security groups take effect immediately</li>
              <li>• Removing rules may disconnect active connections</li>
              <li>• Always test connectivity after making changes</li>
              <li>• Keep SSH access (port 22) open to avoid being locked out</li>
            </ul>
          </div>
            ...
            <Button type="submit" disabled={updating}>
              {updating ? (
                <>
                  ...
                  <span className="ml-2">Updating Security Group...</span>
                </>
              ) : (
                <>
