...
import { X, Maximize2, Minimize2, Terminal as TerminalIcon, Wifi, WifiOff } from 'lucide-react';
...
export default function SSHTerminal({ instanceId, instanceName, keyPairName, host, onClose }: SSHTerminalProps) {
  ...
  const initializeTerminal = () => {
    ...
    terminal.current.writeln('\x1b[1;32m│                    🚀 AWS EC2 SSH Terminal                  │\x1b[0m');
    terminal.current.writeln('\x1b[1;32m╰─────────────────────────────────────────────────────────────╯\x1b[0m');
    terminal.current.writeln('');
    terminal.current.writeln(`\x1b[1;36mConnecting to: \x1b[1;33m${instanceName}\x1b[0m`);
    terminal.current.writeln(`\x1b[1;36mHost: \x1b[1;33m${host}\x1b[0m`);
    terminal.current.writeln(`\x1b[1;36mKey Pair: \x1b[1;33m${keyPairName}\x1b[0m`);
    terminal.current.writeln('');
  };
  ...
  const connectToSSH = () => {
    ...
    socket.current.on('ssh:connected', (data) => {
      ...
      if (terminal.current) {
        terminal.current.writeln(`\x1b[1;32m✅ Connected to ${data.username}@${data.host}\x1b[0m`);
        terminal.current.writeln('');
        terminal.current.writeln('\x1b[1;33m💡 Terminal is ready! You can now type commands.\x1b[0m');
        terminal.current.writeln('');
    ...
    socket.current.on('ssh:disconnected', (data) => {
      ...
      if (terminal.current) {
        ...
        terminal.current.writeln('\x1b[1;31mConnection closed. You can close this terminal.\x1b[0m');
      }
    });
    ...
    socket.current.on('disconnect', () => {
      ...
      if (terminal.current) {
        terminal.current.writeln('\x1b[1;31m🔌 Connection to server lost\x1b[0m');
      }
    });
    ...
    socket.current.on('connect_error', (error) => {
      ...
      if (terminal.current) {
        terminal.current.writeln('\x1b[1;31m❌ Failed to connect to SSH service\x1b[0m');
        terminal.current.writeln('\x1b[1;33mPlease check if the server is running\x1b[0m');
      }
    });
  };
  ...
  return (
    <div className={`fixed bg-white rounded-lg shadow-2xl border border-gray-200 z-50 ${
      isMaximized 
        ? 'inset-4' 
        : 'bottom-4 right-4 w-[900px] h-[650px]'
    }`}>
      {/* Terminal Header */}
      ...
      >
        ...
        {isConnecting && (
          <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center">
            <div className="text-center text-white">
              ...
              <p className="text-lg font-medium">Establishing SSH Connection...</p>
        ...
        {!isConnected && !isConnecting && (
          <div className="absolute bottom-4 left-4 right-4 bg-gray-800 bg-opacity-90 text-white p-3 rounded-lg">
            <p className="text-sm">
              ⚠️ Terminal not connected. Please check the connection status above.
            </p>
          </div>
        )}
      </div>
